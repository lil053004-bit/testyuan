...
export default function LineRedirectsTab() {
  ...
  const handleAddLink = async () => {
    ...
    try {
      const url = new URL(newLink.line_url);
      if (url.protocol !== 'https:') {
        alert('URL必须使用HTTPS协议');
        return;
      }
    } catch {
      alert('请输入有效的URL格式');
  ...
  const handleUpdateLink = async (id: string) => {
    if (!editForm.line_url) {
      alert('请输入URL');
      return;
    }

    try {
      const url = new URL(editForm.line_url);
      if (url.protocol !== 'https:') {
        alert('URL必须使用HTTPS协议');
        return;
      }
  ...
  if (loading) {
    return (
      <div className="text-center py-20">
        ...
        <p className="mt-4 text-slate-900 font-medium">加载分流链接...</p>
        {error && (
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-700 mt-1">创建和管理多个跳转链接，系统将根据权重自动分配流量</p>
        </div>
        <button
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              />
              <p className="text-xs text-slate-500 mt-1">必须使用HTTPS协议</p>
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">
      ...
      <div className="space-y-4">
        {links.length === 0 ? (
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-12 text-center">
            ...
            <p className="text-slate-900 font-medium">还没有创建任何跳转链接</p>
            <p className="text-sm text-slate-700 mt-2">点击上方"添加链接"按钮开始创建</p>
          </div>
        ) : (
          links.map((link) => (
            <div
                ...
                <div className="p-6">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-4 text-sm text-slate-700 ml-8">
                        <div>
