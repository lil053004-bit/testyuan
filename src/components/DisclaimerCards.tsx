...
function DisclaimerCard({ title, content }: DisclaimerCardProps) {
  return (
    <div className="bg-navy-card/60 backdrop-blur-sm border border-yellow-warning/30 rounded-lg overflow-hidden">
      <div className="flex items-start gap-3 px-4 py-3 bg-yellow-warning/10">
        <AlertTriangle className="w-4 h-4 text-yellow-warning flex-shrink-0 mt-0.5" />
        <div>
          <span className="font-bold text-sm text-white block mb-1">{title}</span>
...
export default function DisclaimerCards() {
  const disclaimers = [
    {
      title: '【サービスの性質】',
